...
@immutable
class State {
  const State({required this.bump}) : discriminator = 1;
  ...
  @override
  int get hashCode => Object.hash(discriminator, bump);

  @override
  String toString() => 'State(discriminator: $discriminator, bump: $bump)';
}
...
Encoder<State> getStateEncoder() {
  ...
  return transformEncoder(
    structEncoder,
    (State value) => <String, Object?>{'discriminator': 1, 'bump': value.bump},
  );
}
...
Decoder<State> getStateDecoder() {
  ...
  (State, int) readTopLevel(Uint8List bytes, int offset) {
    ...
    return (State(bump: map['bump']! as int), newOffset);
  }
