...
class PolicyCheckedEvent extends ValidationProgramEvent {
	...
	String toString() => 'PolicyCheckedEvent(discriminator: ${discriminator}, amount: ${amount}, memo: ${memo}, approvals: ${approvals}, requiredApprovals: ${requiredApprovals})';
}
...
PolicyCheckedEvent decodePolicyCheckedEvent(Uint8List data) {
	...
	cursor = c1;
	final (v2, c2) = fixDecoderSize(addDecoderSizePrefix(getUtf8Decoder(), getU8Decoder()), 65).read(data, cursor);
	cursor = c2;
	final (v3, c3) = fixDecoderSize(getArrayDecoder(getU8Decoder(), size: PrefixedArraySize(getU16Decoder())), 6).read(data, cursor);
	cursor = c3;
	final (v4, c4) = getU8Decoder().read(data, cursor);
	cursor = c4;

	return PolicyCheckedEvent(discriminator: v0, amount: v1, memo: v2, approvals: v3, requiredApprovals: v4);
}
