...
class MyEventEvent {
	...
	String toString() => 'MyEventEvent(discriminator: ${discriminator}, migrationVersion: ${migrationVersion}, data: ${data}, label: ${label})';
}
...
MyEventEvent decodeMyEventEvent(Uint8List data) {
	...
	return MyEventEvent(discriminator: v0, migrationVersion: v1, data: v2, label: v3);
}
...
const List<(int, int, bool, int, int, List<(int, int, int)>)> _myEventProjectionSteps = [
...
NormalizedMyEventEvent normalizeMyEventEvent(Uint8List data) {
	if (data.length < 2) {
		throw RangeError('the provided data is too short for the "MyEvent" event envelope');
	}
	for (var index = 0; index < 1; index++) {
		if (data[index] != _myEventEventDiscriminatorBytes[index]) {
			throw RangeError('the provided data does not match the "MyEvent" event discriminator');
		}
	}
