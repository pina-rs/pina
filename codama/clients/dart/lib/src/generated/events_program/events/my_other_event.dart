...
class MyOtherEventEvent {
	...
	String toString() => 'MyOtherEventEvent(discriminator: ${discriminator}, migrationVersion: ${migrationVersion}, data: ${data}, label: ${label})';
}
...
MyOtherEventEvent decodeMyOtherEventEvent(Uint8List data) {
	...
	return MyOtherEventEvent(discriminator: v0, migrationVersion: v1, data: v2, label: v3);
}
...
const List<(int, int, bool, int, int, List<(int, int, int)>)> _myOtherEventProjectionSteps = [
...
NormalizedMyOtherEventEvent normalizeMyOtherEventEvent(Uint8List data) {
	if (data.length < 2) {
		throw RangeError('the provided data is too short for the "MyOtherEvent" event envelope');
	}
	for (var index = 0; index < 1; index++) {
		if (data[index] != _myOtherEventEventDiscriminatorBytes[index]) {
			throw RangeError('the provided data does not match the "MyOtherEvent" event discriminator');
		}
	}
