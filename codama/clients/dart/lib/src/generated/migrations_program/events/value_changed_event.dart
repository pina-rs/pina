...
class ValueChangedEventEvent {
	...
	String toString() => 'ValueChangedEventEvent(discriminator: ${discriminator}, migrationVersion: ${migrationVersion}, value: ${value}, memo: ${memo})';
}
...
ValueChangedEventEvent decodeValueChangedEventEvent(Uint8List data) {
	if (data.length != valueChangedEventEventSize) {
		throw RangeError('expected exactly ${valueChangedEventEventSize} bytes, received ${data.length}');
	}
	var cursor = 0;
	final (v0, c0) = getU8Decoder().read(data, cursor);
	cursor = c0;
	if (v0 != 4) {
		throw RangeError('the provided bytes do not match the "ValueChangedEvent" event discriminator');
	}
	...
	return ValueChangedEventEvent(discriminator: v0, migrationVersion: v1, value: v2, memo: v3);
}
...
const List<(int, int, bool, int, int, List<(int, int, int)>)> _valueChangedEventProjectionSteps = [
	(0, 1, true, 8, 10, [(0, 0, 8)]),
];
...
NormalizedValueChangedEventEvent normalizeValueChangedEventEvent(Uint8List data) {
	if (data.length < 2) {
		throw RangeError('the provided data is too short for the "ValueChangedEvent" event envelope');
	}
	for (var index = 0; index < 1; index++) {
		if (data[index] != _valueChangedEventEventDiscriminatorBytes[index]) {
			throw RangeError('the provided data does not match the "ValueChangedEvent" event discriminator');
		}
	}
...
/// this event.
NormalizedValueChangedEventEvent? parseValueChangedEventEventFromLog(String log) {
