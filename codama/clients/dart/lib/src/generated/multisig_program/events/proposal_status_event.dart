...
class ProposalStatusEventEvent {
	...
	String toString() => 'ProposalStatusEventEvent(discriminator: ${discriminator}, migrationVersion: ${migrationVersion}, multisig: ${multisig}, index: ${index}, status: ${status}, timestamp: ${timestamp})';
}
...
ProposalStatusEventEvent decodeProposalStatusEventEvent(Uint8List data) {
	if (data.length != proposalStatusEventEventSize) {
		throw RangeError('expected exactly ${proposalStatusEventEventSize} bytes, received ${data.length}');
	}
	var cursor = 0;
	final (v0, c0) = getU8Decoder().read(data, cursor);
	cursor = c0;
	if (v0 != 1) {
		throw RangeError('the provided bytes do not match the "ProposalStatusEvent" event discriminator');
	}
	...
	return ProposalStatusEventEvent(discriminator: v0, migrationVersion: v1, multisig: v2, index: v3, status: v4, timestamp: v5);
}
...
const List<(int, int, bool, int, int, List<(int, int, int)>)> _proposalStatusEventProjectionSteps = [
...
NormalizedProposalStatusEventEvent normalizeProposalStatusEventEvent(Uint8List data) {
	if (data.length < 2) {
		throw RangeError('the provided data is too short for the "ProposalStatusEvent" event envelope');
	}
	for (var index = 0; index < 1; index++) {
		if (data[index] != _proposalStatusEventEventDiscriminatorBytes[index]) {
			throw RangeError('the provided data does not match the "ProposalStatusEvent" event discriminator');
		}
	}
...
/// this event.
NormalizedProposalStatusEventEvent? parseProposalStatusEventEventFromLog(String log) {
