...
use clap::Args;
use profile_program_client::instructions::AddTag;
use profile_program_client::instructions::AddTagInstructionData;
...
pub(crate) fn run(context: &CliContext, args: AddTagArgs) -> Result<(), CliError> {
	...
	let data = AddTagInstructionData::new(|data| {
		data.tag.set(args.tag);
	})
	.map_err(|_| {
