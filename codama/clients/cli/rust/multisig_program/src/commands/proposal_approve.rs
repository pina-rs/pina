...
use clap::Args;
use multisig_program_client::instructions::ProposalApprove;
use multisig_program_client::instructions::ProposalApproveInstructionData;
...
pub(crate) fn run(context: &CliContext, args: ProposalApproveArgs) -> Result<(), CliError> {
	...
	let data = ProposalApproveInstructionData::new(|_data| {}).map_err(|_| {
		CliError::InvalidData {
			message: "instruction data rejected the provided arguments".to_string(),
		}
	})?;

	let accounts = ProposalApprove {
		multisig,
		proposal,
		member,
		clock,
