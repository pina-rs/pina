...
use clap::Args;
use multisig_program_client::instructions::ProposalActivate;
use multisig_program_client::instructions::ProposalActivateInstructionData;
...
pub(crate) fn run(context: &CliContext, args: ProposalActivateArgs) -> Result<(), CliError> {
	...
	let data = ProposalActivateInstructionData::new(|_data| {}).map_err(|_| {
		CliError::InvalidData {
			message: "instruction data rejected the provided arguments".to_string(),
		}
	})?;

	let accounts = ProposalActivate {
		multisig,
		proposal,
		member,
		clock,
