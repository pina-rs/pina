...
export function getOpenPositionDiscriminatorBytes(): ReadonlyUint8Array { return getU8Encoder().encode(OPEN_POSITION_DISCRIMINATOR); }
...
export type OpenPositionInstructionData = { discriminator: number; bump: number;  };
...
export function getOpenPositionInstructionDataDecoder(): FixedSizeDecoder<OpenPositionInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(OPEN_POSITION_DISCRIMINATOR, getU8Decoder())], ['bump', getU8Decoder()]]);
}
...
const programAddress = config?.programAddress ?? STAKING_REWARDS_PROGRAM_PROGRAM_ADDRESS;
 ...
 // Original accounts.
const originalAccounts = { user: { value: input.user ?? null, isWritable: true }, poolState: { value: input.poolState ?? null, isWritable: false }, positionState: { value: input.positionState ?? null, isWritable: true }, systemProgram: { value: input.systemProgram ?? null, isWritable: false } }
