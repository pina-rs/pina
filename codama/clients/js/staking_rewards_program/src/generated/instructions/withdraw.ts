...
export function getWithdrawDiscriminatorBytes(): ReadonlyUint8Array { return getU8Encoder().encode(WITHDRAW_DISCRIMINATOR); }
...
export function getWithdrawInstructionDataEncoder(): FixedSizeEncoder<WithdrawInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', getU8Encoder()], ['amount', getU64Encoder()]]), (value) => ({ ...value, discriminator: 3 }));
}

export function getWithdrawInstructionDataDecoder(): FixedSizeDecoder<WithdrawInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(WITHDRAW_DISCRIMINATOR, getU8Decoder())], ['amount', getU64Decoder()]]);
}
...
return Object.freeze({ accounts: [getAccountMeta("user", accounts.user), getAccountMeta("stakeMint", accounts.stakeMint), getAccountMeta("poolState", accounts.poolState), getAccountMeta("positionState", accounts.positionState), getAccountMeta("userStakeAta", accounts.userStakeAta), getAccountMeta("tokenProgram", accounts.tokenProgram), getAccountMeta("systemProgram", accounts.systemProgram)], data: getWithdrawInstructionDataEncoder().encode(args as WithdrawInstructionDataArgs), programAddress } as WithdrawInstruction<TProgramAddress, TAccountUser, TAccountStakeMint, TAccountPoolState, TAccountPositionState, TAccountUserStakeAta, TAccountTokenProgram, TAccountSystemProgram>);
}

export type ParsedWithdrawInstruction<TProgram extends string = typeof STAKING_REWARDS_PROGRAM_PROGRAM_ADDRESS, TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[]> = { programAddress: Address<TProgram>;
accounts: {
