...
export function getDirectTransferDiscriminatorBytes(): ReadonlyUint8Array { return getU8Encoder().encode(DIRECT_TRANSFER_DISCRIMINATOR); }

export type DirectTransferInstruction<TProgram extends string = typeof TRANSFER_SOL_PROGRAM_PROGRAM_ADDRESS, TAccountSender extends string | AccountMeta<string> = string, TAccountRecipient extends string | AccountMeta<string> = string, TRemainingAccounts extends readonly AccountMeta<string>[] = []> =
...
export type DirectTransferInstructionData = { discriminator: number; amount: bigint;  };
...
export function getDirectTransferInstructionDataDecoder(): FixedSizeDecoder<DirectTransferInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(DIRECT_TRANSFER_DISCRIMINATOR, getU8Decoder())], ['amount', getU64Decoder()]]);
}
...
export type ParsedDirectTransferInstruction<TProgram extends string = typeof TRANSFER_SOL_PROGRAM_PROGRAM_ADDRESS, TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[]> = { programAddress: Address<TProgram>;
accounts: {
/** The sender. Must be owned by this program, writable, and a signer. */
