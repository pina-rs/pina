...
export function getOracleStateDiscriminatorBytes(): ReadonlyUint8Array { return getU8Encoder().encode(ORACLE_STATE_DISCRIMINATOR); }

export type OracleState = { discriminator: number; authority: Address; price: bigint;  };
...
export function getOracleStateEncoder(): FixedSizeEncoder<OracleStateArgs> {
    return transformEncoder(getStructEncoder([['discriminator', getU8Encoder()], ['authority', getAddressEncoder()], ['price', getU64Encoder()]]), (value) => ({ ...value, discriminator: 1 }));
}

/** Gets the decoder for {@link OracleState} account data. */
export function getOracleStateDecoder(): FixedSizeDecoder<OracleState> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(ORACLE_STATE_DISCRIMINATOR, getU8Decoder())], ['authority', getAddressDecoder()], ['price', getU64Decoder()]]);
}
