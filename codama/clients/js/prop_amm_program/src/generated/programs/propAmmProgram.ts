...
export type PropAmmProgramPlugin = { accounts: PropAmmProgramPluginAccounts; instructions: PropAmmProgramPluginInstructions; identifyAccount: typeof identifyPropAmmProgramAccount; identifyInstruction: typeof identifyPropAmmProgramInstruction; parseInstruction: typeof parsePropAmmProgramInstruction; }
...
export function propAmmProgramProgram() {
    return <T extends PropAmmProgramPluginRequirements>(client: T): ExtendedClient<T, { propAmmProgram: PropAmmProgramPlugin }> => {
        return extendClient(client, { propAmmProgram: <PropAmmProgramPlugin>{ accounts: { oracleState: addSelfFetchFunctions(client, getOracleStateCodec()) }, instructions: { initialize: input => addSelfPlanAndSendFunctions(client, getInitializeInstruction(input)), update: input => addSelfPlanAndSendFunctions(client, getUpdateInstruction(input)), rotateAuthority: input => addSelfPlanAndSendFunctions(client, getRotateAuthorityInstruction(input)) }, identifyAccount: identifyPropAmmProgramAccount, identifyInstruction: identifyPropAmmProgramInstruction, parseInstruction: parsePropAmmProgramInstruction } });
    };
}