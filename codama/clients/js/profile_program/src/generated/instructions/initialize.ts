...
export type InitializeInstructionData = { discriminator: number; bump: number; name: string; bio: string;  };

export type InitializeInstructionDataArgs = { bump: number; name: string; bio: string;  };

export function getInitializeInstructionDataEncoder(): FixedSizeEncoder<InitializeInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', getU8Encoder()], ['bump', getU8Encoder()], ['name', fixPinaPodEncoderSize(addEncoderSizePrefix(getUtf8Encoder(), getU8Encoder()), 33)], ['bio', fixPinaPodEncoderSize(addEncoderSizePrefix(getUtf8Encoder(), getU8Encoder()), 129)]]), (value) => ({ ...value, discriminator: 0 }));
}

export function getInitializeInstructionDataDecoder(): FixedSizeDecoder<InitializeInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(INITIALIZE_DISCRIMINATOR, getU8Decoder())], ['bump', getU8Decoder()], ['name', getPinaPodStringDecoder(getU8Decoder(), 33)], ['bio', getPinaPodStringDecoder(getU8Decoder(), 129)]]);
}
