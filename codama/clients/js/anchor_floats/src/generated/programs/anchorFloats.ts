...
export const ANCHOR_FLOATS_PROGRAM_ADDRESS =
	"Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS" as Address<
		"Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS"
	>;

export enum AnchorFloatsAccount {
	FloatDataAccount,
}

export function identifyAnchorFloatsAccount(
	account: { data: ReadonlyUint8Array } | ReadonlyUint8Array,
): AnchorFloatsAccount {
	const data = "data" in account ? account.data : account;
	if (containsBytes(data, getU8Encoder().encode(1), 0)) {
		return AnchorFloatsAccount.FloatDataAccount;
	}
	throw new SolanaError(
		SOLANA_ERROR__PROGRAM_CLIENTS__FAILED_TO_IDENTIFY_ACCOUNT,
		{ accountData: data, programName: "anchorFloats" },
	);
}

export enum AnchorFloatsInstruction {
	Create,
	Update,
}

export function identifyAnchorFloatsInstruction(
	instruction: { data: ReadonlyUint8Array } | ReadonlyUint8Array,
): AnchorFloatsInstruction {
	...
	throw new SolanaError(
		SOLANA_ERROR__PROGRAM_CLIENTS__FAILED_TO_IDENTIFY_INSTRUCTION,
		{ instructionData: data, programName: "anchorFloats" },
	);
}

export type ParsedAnchorFloatsInstruction<
	TProgram extends string = "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS",
> =
	| { instructionType: AnchorFloatsInstruction.Create }
		& ParsedCreateInstruction<TProgram>
	| { instructionType: AnchorFloatsInstruction.Update }
...
export function parseAnchorFloatsInstruction<TProgram extends string>(
	...
	switch (instructionType) {
		case AnchorFloatsInstruction.Create: {
		...
		case AnchorFloatsInstruction.Update: {
			assertIsInstructionWithAccounts(instruction);
			return {
				instructionType: AnchorFloatsInstruction.Update,
...
export type AnchorFloatsPluginInstructions = {
	create: (
		input: CreateInput,
...
export type AnchorFloatsPluginRequirements =
