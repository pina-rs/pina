...
export type InitializeInstructionData = { discriminator: number; bump: number; entryCount: number; markerCount: number;  };

export type InitializeInstructionDataArgs = { bump: number; entryCount: number; markerCount: number;  };

export function getInitializeInstructionDataEncoder(): FixedSizeEncoder<InitializeInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', getU8Encoder()], ['bump', getU8Encoder()], ['entryCount', getU8Encoder()], ['markerCount', getU8Encoder()]]), (value) => ({ ...value, discriminator: 0 }));
}

export function getInitializeInstructionDataDecoder(): FixedSizeDecoder<InitializeInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(INITIALIZE_DISCRIMINATOR, getU8Decoder())], ['bump', getU8Decoder()], ['entryCount', getU8Decoder()], ['markerCount', getU8Decoder()]]);
}
...
export type InitializeAsyncInput<TAccountAuthority extends string = string, TAccountJournal extends string = string, TAccountSystemProgram extends string = string> =  {
...
export async function getInitializeInstructionAsync<TAccountAuthority extends string, TAccountJournal extends string, TAccountSystemProgram extends string, TProgramAddress extends Address = typeof COMPACT_ACCOUNTS_PROGRAM_PROGRAM_ADDRESS>(input: InitializeAsyncInput<TAccountAuthority, TAccountJournal, TAccountSystemProgram>, config?: { programAddress?: TProgramAddress } ): Promise<InitializeInstruction<TProgramAddress, TAccountAuthority, TAccountJournal, TAccountSystemProgram>> {
  // Program address.
...
return Object.freeze({ accounts: [getAccountMeta("authority", accounts.authority), getAccountMeta("journal", accounts.journal), getAccountMeta("systemProgram", accounts.systemProgram)], data: getInitializeInstructionDataEncoder().encode(args as InitializeInstructionDataArgs), programAddress } as InitializeInstruction<TProgramAddress, TAccountAuthority, TAccountJournal, TAccountSystemProgram>);
}

export type InitializeInput<TAccountAuthority extends string = string, TAccountJournal extends string = string, TAccountSystemProgram extends string = string> =  {
  /** Funds rent and becomes the journal authority. */
authority: TransactionSigner<TAccountAuthority>;
/** Empty canonical journal PDA. */
journal: Address<TAccountJournal>;
...
export function getInitializeInstruction<TAccountAuthority extends string, TAccountJournal extends string, TAccountSystemProgram extends string, TProgramAddress extends Address = typeof COMPACT_ACCOUNTS_PROGRAM_PROGRAM_ADDRESS>(input: InitializeInput<TAccountAuthority, TAccountJournal, TAccountSystemProgram>, config?: { programAddress?: TProgramAddress } ): InitializeInstruction<TProgramAddress, TAccountAuthority, TAccountJournal, TAccountSystemProgram> {
  // Program address.
const programAddress = config?.programAddress ?? COMPACT_ACCOUNTS_PROGRAM_PROGRAM_ADDRESS;
...
export type ParsedInitializeInstruction<TProgram extends string = typeof COMPACT_ACCOUNTS_PROGRAM_PROGRAM_ADDRESS, TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[]> = { programAddress: Address<TProgram>;
accounts: {
/** Funds rent and becomes the journal authority. */
...
const getNextAccount = () => {
  ...
  return { programAddress: instruction.programAddress, accounts: { authority: getNextAccount(), journal: getNextAccount(), systemProgram: getNextAccount() }, data: getInitializeInstructionDataDecoder().decode(instruction.data) };
}