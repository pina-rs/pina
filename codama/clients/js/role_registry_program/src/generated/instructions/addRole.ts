...
export function getAddRoleDiscriminatorBytes(): ReadonlyUint8Array { return getU8Encoder().encode(ADD_ROLE_DISCRIMINATOR); }
...
export type AddRoleInstructionData = { discriminator: number; roleId: bigint; permissions: bigint; bump: number;  };
...
export function getAddRoleInstructionDataEncoder(): FixedSizeEncoder<AddRoleInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', getU8Encoder()], ['roleId', getU64Encoder()], ['permissions', getU64Encoder()], ['bump', getU8Encoder()]]), (value) => ({ ...value, discriminator: 1 }));
}

export function getAddRoleInstructionDataDecoder(): FixedSizeDecoder<AddRoleInstructionData> {
    return getStructDecoder([['discriminator', getPinaPodDiscriminatorDecoder(ADD_ROLE_DISCRIMINATOR, getU8Decoder())], ['roleId', getU64Decoder()], ['permissions', getU64Decoder()], ['bump', getU8Decoder()]]);
}
...
const programAddress = config?.programAddress ?? ROLE_REGISTRY_PROGRAM_PROGRAM_ADDRESS;
 ...
 // Original accounts.
const originalAccounts = { admin: { value: input.admin ?? null, isWritable: true }, grantee: { value: input.grantee ?? null, isWritable: false }, registryConfig: { value: input.registryConfig ?? null, isWritable: true }, roleEntry: { value: input.roleEntry ?? null, isWritable: true }, systemProgram: { value: input.systemProgram ?? null, isWritable: false } }
