...
export function getRoleEntryDiscriminatorBytes() { return getU8Encoder().encode(ROLE_ENTRY_DISCRIMINATOR); }

export type RoleEntry = { registry: Address; roleId: bigint; grantee: Address; permissions: bigint; active: boolean; bump: number;  };
...
export function getRoleEntryEncoder(): FixedSizeEncoder<RoleEntryArgs> {
    return getStructEncoder([['registry', getAddressEncoder()], ['roleId', getU64Encoder()], ['grantee', getAddressEncoder()], ['permissions', getU64Encoder()], ['active', getBooleanEncoder()], ['bump', getU8Encoder()]]);
}

/** Gets the decoder for {@link RoleEntry} account data. */
export function getRoleEntryDecoder(): FixedSizeDecoder<RoleEntry> {
    return getStructDecoder([['registry', getAddressDecoder()], ['roleId', getU64Decoder()], ['grantee', getAddressDecoder()], ['permissions', getU64Decoder()], ['active', getBooleanDecoder()], ['bump', getU8Decoder()]]);
}
