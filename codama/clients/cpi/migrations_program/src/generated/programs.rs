...
/// Whether `address` is the `migrationsProgram` program this crate calls.
///
...
mod tests {
	...
	fn binds_the_expected_program_id() {
		assert_eq!(
			MIGRATIONS_PROGRAM_ID,
			pina::address!("GJQcuWrT2f3f4KNuJcXhhwUa1ZQTYbxzzJ1hotzKu8hS")
		);
		assert_eq!(MigrationsProgram::ID, MIGRATIONS_PROGRAM_ID);
		assert!(is_expected_program(&MIGRATIONS_PROGRAM_ID));
	}

	#[test]
	fn rejects_a_foreign_program_id() {
		let mut foreign_bytes = MIGRATIONS_PROGRAM_ID.to_bytes();
		foreign_bytes[0] ^= 0xFF;
		let foreign = pina::Address::new_from_array(foreign_bytes);
		assert_ne!(foreign, MIGRATIONS_PROGRAM_ID);
