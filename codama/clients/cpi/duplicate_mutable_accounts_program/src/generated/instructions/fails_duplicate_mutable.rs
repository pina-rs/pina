...
/// CPI call for the `fails_duplicate_mutable` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct FailsDuplicateMutable<'account> {
...
impl FailsDuplicateMutableIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&FAILS_DUPLICATE_MUTABLE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> FailsDuplicateMutable<'account> {
...
const FAILS_DUPLICATE_MUTABLE_DISCRIMINATOR: [u8; 2] = [0, 0];
