...
/// Whether `address` is the `optionalAccountsProgram` program this crate calls.
///
...
pub fn is_expected_program(address: &Address) -> bool {
	*address == OPTIONAL_ACCOUNTS_PROGRAM_ID
}
...
mod tests {
	...
	fn binds_the_expected_program_id() {
		assert_eq!(
			OPTIONAL_ACCOUNTS_PROGRAM_ID,
			pina::address!("ccdMMVpwebk8NxwJdY4CndxkLKUTM6fkaFUteAfFeci")
		);
		assert_eq!(OptionalAccountsProgram::ID, OPTIONAL_ACCOUNTS_PROGRAM_ID);
		assert!(is_expected_program(&OPTIONAL_ACCOUNTS_PROGRAM_ID));
	}

	#[test]
	fn rejects_a_foreign_program_id() {
		let mut foreign_bytes = OPTIONAL_ACCOUNTS_PROGRAM_ID.to_bytes();
		foreign_bytes[0] ^= 0xFF;
		let foreign = pina::Address::new_from_array(foreign_bytes);
		assert_ne!(foreign, OPTIONAL_ACCOUNTS_PROGRAM_ID);
