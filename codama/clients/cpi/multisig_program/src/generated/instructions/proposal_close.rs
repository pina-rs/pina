...
/// CPI call for the `proposal_close` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct ProposalClose<'account> {
...
/// Instruction arguments for the `proposal_close` CPI call.
#[derive(Clone, Copy, Debug, Default)]
pub struct ProposalCloseIx;

impl ProposalCloseIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&PROPOSAL_CLOSE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> ProposalClose<'account> {
...
const PROPOSAL_CLOSE_DISCRIMINATOR: [u8; 2] = [14, 0];
