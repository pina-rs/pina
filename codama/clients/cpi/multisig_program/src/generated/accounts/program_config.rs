//! This code was AUTOGENERATED by `pina_cpi_renderer`.
//! Please DO NOT EDIT THIS FILE directly.
//! Regenerate it from Codama IDLs instead.
#![allow(
	clippy::doc_lazy_continuation,
	clippy::empty_line_after_doc_comments,
	clippy::too_many_arguments
)]
#![allow(rustdoc::broken_intra_doc_links)]

use pina::Address;

#[allow(unused_imports)]
use crate::generated_types::*;

/// Global program configuration: the authority that may update it, the
/// treasury that collects multisig creation fees, and the fee itself.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ProgramConfig {
	pub bump: u8,
	pub authority: Address,
	pub treasury: Address,
	pub creation_fee: u64,
}

/// Account discriminator declared by the program's IDL.
pub const PROGRAM_CONFIG_DISCRIMINATOR: [u8; 2] = [1, 0];

impl ProgramConfig {
	/// Encoded size of this account's data.
	pub const LEN: usize = 75;

	/// Whether `data` carries this account's discriminator.
	#[inline(always)]
	pub fn matches(data: &[u8]) -> bool {
		data.len() >= 2 && data[..2] == PROGRAM_CONFIG_DISCRIMINATOR
	}

	/// Reads this account's fields from the account's data.
	///
	/// Returns `None` when the data is too short, the discriminator does not match, or a field is truncated.
	#[inline(always)]
	pub fn parse(data: &[u8]) -> Option<ProgramConfig> {
		if !Self::matches(data) {
			return None;
		}

		let mut cursor = 0usize;
		cursor += 2;

		let bump: u8 = u8::from_le_bytes(data.get(cursor..cursor + 1)?.try_into().ok()?);
		cursor += 1;
		let authority = Address::new_from_array(data.get(cursor..cursor + 32)?.try_into().ok()?);
		cursor += 32;
		let treasury = Address::new_from_array(data.get(cursor..cursor + 32)?.try_into().ok()?);
		cursor += 32;
		let creation_fee: u64 = u64::from_le_bytes(data.get(cursor..cursor + 8)?.try_into().ok()?);

		Some(ProgramConfig {
			bump,
			authority,
			treasury,
			creation_fee,
		})
	}
}
