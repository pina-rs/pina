...
/// CPI call for the `allocate` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct Allocate {
	/// Instruction arguments encoded and sent as CPI data for `allocate`.
	pub ix: AllocateIx,
}

/// Instruction arguments for the `allocate` CPI call.
#[derive(Clone, Copy, Debug)]
...
impl AllocateIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 10], ProgramError> {
		let mut data = [0u8; 10];
		data[..2].copy_from_slice(&ALLOCATE_DISCRIMINATOR);
...
const ALLOCATE_DISCRIMINATOR: [u8; 2] = [0, 0];
