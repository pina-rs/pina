...
/// CPI call for the `deactivate_role` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct DeactivateRole<'account> {
	/// CPI account `admin`.
...
impl DeactivateRoleIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&DEACTIVATE_ROLE_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> DeactivateRole<'account> {
...
const DEACTIVATE_ROLE_DISCRIMINATOR: [u8; 2] = [3, 0];
