...
/// CPI call for the `challenge_disclosure` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct ChallengeDisclosure<'account> {
	...
	/// Instruction arguments encoded and sent as CPI data for `challenge_disclosure`.
	pub ix: ChallengeDisclosureIx,
}

/// Instruction arguments for the `challenge_disclosure` CPI call.
#[derive(Clone, Copy, Debug)]
...
impl ChallengeDisclosureIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 3], ProgramError> {
		let mut data = [0u8; 3];
		data[..2].copy_from_slice(&CHALLENGE_DISCLOSURE_DISCRIMINATOR);
...
impl<'account> ChallengeDisclosure<'account> {
...
const CHALLENGE_DISCLOSURE_DISCRIMINATOR: [u8; 2] = [9, 0];
