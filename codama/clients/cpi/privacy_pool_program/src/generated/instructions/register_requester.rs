//! This code was AUTOGENERATED by `pina_cpi_renderer`.
//! Please DO NOT EDIT THIS FILE directly.
//! Regenerate it from Codama IDLs instead.
#![allow(
	clippy::doc_lazy_continuation,
	clippy::empty_line_after_doc_comments,
	clippy::too_many_arguments
)]
#![allow(rustdoc::broken_intra_doc_links)]

use pina::AccountView;
use pina::Address;
use pina::CpiContext;
use pina::CpiHandle;
use pina::ProgramError;
use pina::ProgramResult;
use pina::Signer;

use crate::ProgramAccount;

/// CPI call for the `register_requester` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct RegisterRequester<'account, 'argument> {
	/// CPI account `authority`.
	/// Required privileges: read-only and signer.
	pub authority: &'account AccountView,

	/// CPI account `poolConfig`.
	/// Required privileges: read-only.
	pub pool_config: &'account AccountView,

	/// CPI account `requesterRegistry`.
	/// Required privileges: writable.
	pub requester_registry: &'account AccountView,

	/// Instruction arguments encoded and sent as CPI data for `register_requester`.
	pub ix: RegisterRequesterIx<'argument>,
}

/// Instruction arguments for the `register_requester` CPI call.
#[derive(Clone, Copy, Debug)]
pub struct RegisterRequesterIx<'argument> {
	/// Instruction argument `requester`.
	pub requester: &'argument Address,

	/// Instruction argument `maxTier`.
	pub max_tier: u8,
}

impl<'argument> RegisterRequesterIx<'argument> {
	/// Number of bytes in the encoded instruction, including its discriminator.
	pub const LEN: usize = 35;

	/// Encodes the discriminator and instruction arguments for CPI.
	#[inline(always)]
	pub fn to_bytes(&self) -> Result<[u8; 35], ProgramError> {
		let mut data = [0u8; 35];
		data[..2].copy_from_slice(&REGISTER_REQUESTER_DISCRIMINATOR);
		data[2..34].copy_from_slice(self.requester.as_ref());
		data[34..35].copy_from_slice(&self.max_tier.to_le_bytes());

		Ok(data)
	}
}

impl<'account, 'argument> RegisterRequester<'account, 'argument> {
	/// Invokes the instruction with no PDA seeds.
	#[inline(always)]
	pub fn invoke(&self, program: &ProgramAccount<'_>) -> ProgramResult {
		self.invoke_signed(program, &[])
	}

	/// Invokes the instruction, signing with the provided PDA seeds.
	#[inline(always)]
	pub fn invoke_signed(
		&self,
		program: &ProgramAccount<'_>,
		signers: &[Signer<'_, '_>],
	) -> ProgramResult {
		let accounts: [CpiHandle<'_>; 3] = [
			CpiHandle::readonly_signer(self.authority),
			CpiHandle::readonly(self.pool_config),
			CpiHandle::writable(self.requester_registry)?,
		];
		let data = self.ix.to_bytes()?;
		let context = CpiContext::new(*program, accounts);

		context.invoke_signed(&data, signers)
	}
}

const REGISTER_REQUESTER_DISCRIMINATOR: [u8; 2] = [3, 0];
