...
/// CPI call for the `sysvars` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct Sysvars<'account> {
	/// CPI account `clock`.
	...
	/// CPI account `rent`.
	/// Required privileges: read-only.
	pub rent: &'account AccountView,

	/// CPI account `stakeHistory`.
	/// Required privileges: read-only.
...
impl SysvarsIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&SYSVARS_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> Sysvars<'account> {
...
const SYSVARS_DISCRIMINATOR: [u8; 2] = [0, 0];
