...
pub struct SysvarChecksProgram;

impl CpiProgramId for SysvarChecksProgram {
	const ID: Address = SYSVAR_CHECKS_PROGRAM_ID;
}

/// A validated executable account for the `sysvarChecksProgram` program.
pub type ProgramAccount<'a> = Program<'a, SysvarChecksProgram>;

/// Whether `address` is the `sysvarChecksProgram` program this crate calls.
///
...
pub fn is_expected_program(address: &Address) -> bool {
	*address == SYSVAR_CHECKS_PROGRAM_ID
}
...
mod tests {
	...
	fn binds_the_expected_program_id() {
		assert_eq!(
			SYSVAR_CHECKS_PROGRAM_ID,
			pina::address!("Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS")
		);
		assert_eq!(SysvarChecksProgram::ID, SYSVAR_CHECKS_PROGRAM_ID);
		assert!(is_expected_program(&SYSVAR_CHECKS_PROGRAM_ID));
	}

	#[test]
	fn rejects_a_foreign_program_id() {
		let mut foreign_bytes = SYSVAR_CHECKS_PROGRAM_ID.to_bytes();
		foreign_bytes[0] ^= 0xFF;
		let foreign = pina::Address::new_from_array(foreign_bytes);
		assert_ne!(foreign, SYSVAR_CHECKS_PROGRAM_ID);
