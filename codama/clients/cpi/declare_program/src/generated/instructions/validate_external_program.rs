...
/// CPI call for the `validate_external_program` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct ValidateExternalProgram<'account> {
	/// CPI account `authority`.
	/// Required privileges: read-only and signer.
	pub authority: &'account AccountView,

	/// CPI account `externalProgram`.
...
impl ValidateExternalProgramIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&VALIDATE_EXTERNAL_PROGRAM_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> ValidateExternalProgram<'account> {
...
const VALIDATE_EXTERNAL_PROGRAM_DISCRIMINATOR: [u8; 2] = [0, 0];
