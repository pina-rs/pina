...
/// CPI call for the `toggle_completed` instruction.
#[derive(Clone, Copy, Debug)]
#[must_use = "the CPI has no effect until invoke or invoke_signed is called"]
pub struct ToggleCompleted<'account> {
	/// CPI account `owner`.
...
impl ToggleCompletedIx {
	...
	pub fn to_bytes(&self) -> Result<[u8; 2], ProgramError> {
		let mut data = [0u8; 2];
		data[..2].copy_from_slice(&TOGGLE_COMPLETED_DISCRIMINATOR);

		Ok(data)
	}
}

impl<'account> ToggleCompleted<'account> {
...
const TOGGLE_COMPLETED_DISCRIMINATOR: [u8; 2] = [1, 0];
