...
pub const REQUESTER_REGISTRY_DISCRIMINATOR: u8 = 6u8;

pub const REQUESTER_REGISTRY_MIGRATION_VERSION: u8 = 0u8;

impl RequesterRegistry {
	pub const LEN: usize = core::mem::size_of::<RequesterRegistryZc>();
	...
	pub fn initialize(
		data: &mut [u8],
		configure: impl FnOnce(&mut RequesterRegistryZc),
	) -> Result<&mut RequesterRegistryZc, solana_program_error::ProgramError> {
		<Self as pina::PinaPodFixed>::initialize(data, |account| {
			configure(account);
			account.discriminator = REQUESTER_REGISTRY_DISCRIMINATOR;
			account.migration_version = REQUESTER_REGISTRY_MIGRATION_VERSION;
			Ok(())
		})
	...
	pub fn from_bytes(
		data: &[u8],
	) -> Result<&RequesterRegistryZc, solana_program_error::ProgramError> {
		let account = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| solana_program_error::ProgramError::InvalidAccountData)?;
		if account.discriminator != REQUESTER_REGISTRY_DISCRIMINATOR {
			return Err(solana_program_error::ProgramError::InvalidAccountData);
		}
		if account.migration_version != REQUESTER_REGISTRY_MIGRATION_VERSION {
			return Err(solana_program_error::ProgramError::InvalidAccountData);
		}
		Ok(account)
	}
...
impl RequesterRegistry {
	...
	pub fn create_pda(bump: u8) -> Result<solana_pubkey::Pubkey, solana_pubkey::PubkeyError> {
		solana_pubkey::Pubkey::create_program_address(
			&["privacy-pool-requesters".as_bytes(), &[bump]],
			&crate::PRIVACY_POOL_PROGRAM_ID,
		)
	}
}
...
/// [`REQUESTER_REGISTRY_MIGRATION_VERSION`]. Current or foreign bytes return false; decoding explains the difference.
///
/// Version 0 is the initial version, so no bytes can ever be stale.
pub fn requester_registry_needs_migration(_data: &[u8]) -> bool {
	false
}

/// Why `RequesterRegistry::try_from_bytes` rejected account bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RequesterRegistryVersionError {
...
impl core::fmt::Display for RequesterRegistryVersionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid RequesterRegistry account data"),
			Self::Stale { stored } => {
				write!(
					f,
...
impl RequesterRegistry {
	/// Decodes current-version bytes and tells stale envelopes (migrate the account) apart from future ones (upgrade this client). The failure message mirrors the generated JavaScript decoder. For the strict current-only convenience returning `ProgramError`, see [`RequesterRegistry::from_bytes`].
	pub fn try_from_bytes(
		data: &[u8],
	) -> Result<&RequesterRegistryZc, RequesterRegistryVersionError> {
		let account = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| RequesterRegistryVersionError::InvalidData)?;
		if account.discriminator != REQUESTER_REGISTRY_DISCRIMINATOR {
			return Err(RequesterRegistryVersionError::InvalidData);
		}
		if account.migration_version > REQUESTER_REGISTRY_MIGRATION_VERSION {
			return Err(RequesterRegistryVersionError::Future {
...
#[cfg(test)]
mod requester_registry_version_error_tests {
	use super::*;

	fn envelope(version: u8) -> Vec<u8> {
		let mut data = vec![0_u8; core::mem::size_of::<RequesterRegistryZc>()];
	...
	fn stale_and_future_versions_are_distinguishable() {
		let error = RequesterRegistry::try_from_bytes(&envelope(1 as u8))
			.err()
			.expect("a future envelope must fail");
		assert_eq!(error, RequesterRegistryVersionError::Future { stored: 1 });
		assert_eq!(
			RequesterRegistryVersionError::Future { stored: 1 }.to_string(),
			"migration version mismatch: expected 0, received 1 (the data was written by a newer \
			 program; upgrade this client)"
		);
		assert!(
			RequesterRegistry::try_from_bytes(&envelope(0 as u8)).is_ok(),
