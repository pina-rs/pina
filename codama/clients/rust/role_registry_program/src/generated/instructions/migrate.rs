...
pub struct Migrate {
	...
	pub registry_config: Option<solana_pubkey::Pubkey>,
	pub role_entry: Option<solana_pubkey::Pubkey>,
}
...
impl Migrate {
	...
	pub fn instruction_with_remaining_accounts(
		...
		if let Some(registry_config) = self.registry_config {
			accounts.push(solana_instruction::AccountMeta::new(registry_config, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::ROLE_REGISTRY_PROGRAM_ID,
				false,
		...
		if let Some(role_entry) = self.role_entry {
			accounts.push(solana_instruction::AccountMeta::new(role_entry, false));
		} else {
			accounts.push(solana_instruction::AccountMeta::new_readonly(
				crate::ROLE_REGISTRY_PROGRAM_ID,
				false,
