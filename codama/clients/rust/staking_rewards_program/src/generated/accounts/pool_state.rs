...
impl pina::PinaSerialize for PoolState {
	fn write_bytes(&self, output: &mut [u8]) {
		...
		pina::PinaSerialize::write_bytes(&self.paused, &mut output[offset..offset + field_size]);
		offset += field_size;
		let field_size = core::mem::size_of::<u8>();
...
impl pina::ZcValidate for PoolState {
	fn validate_ref(value: &Self) -> Result<(), pina::ZeroPodError> {
		...
		<solana_pubkey::Pubkey as pina::ZcValidate>::validate_ref(&value.stake_mint)?;
		<solana_pubkey::Pubkey as pina::ZcValidate>::validate_ref(&value.reward_mint)?;
		<pina::PodU64 as pina::ZcValidate>::validate_ref(&value.total_staked)?;
		<pina::PodU64 as pina::ZcValidate>::validate_ref(&value.reward_index)?;
		<pina::PodBool as pina::ZcValidate>::validate_ref(&value.paused)?;
...
#[allow(unsafe_code)]
unsafe impl pina::ZcElem for PoolState {}
