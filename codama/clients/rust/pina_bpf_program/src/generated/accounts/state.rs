...
pub const STATE_MIGRATION_VERSION: u8 = 0u8;
...
pub fn state_needs_migration(_data: &[u8]) -> bool {
	false
}
...
mod state_version_error_tests {
	...
	fn stale_and_future_versions_are_distinguishable() {
		let error = State::try_from_bytes(&envelope(1 as u8))
			.err()
			.expect("a future envelope must fail");
		assert_eq!(error, StateVersionError::Future { stored: 1 });
		assert_eq!(
			StateVersionError::Future { stored: 1 }.to_string(),
			"migration version mismatch: expected 0, received 1 (the data was written by a newer \
			 program; upgrade this client)"
		);
		assert!(
			State::try_from_bytes(&envelope(0 as u8)).is_ok(),
