...
pub const PROPOSAL_STATUS_EVENT_DISCRIMINATOR: u8 = 1u8;

pub const PROPOSAL_STATUS_EVENT_MIGRATION_VERSION: u8 = 0u8;

impl ProposalStatusEvent {
	/// Exact size of the current event representation.
	pub const LEN: usize = core::mem::size_of::<ProposalStatusEventZc>();
	...
	pub fn from_bytes(
		data: &[u8],
	) -> Result<&ProposalStatusEventZc, solana_program_error::ProgramError> {
		let event = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| solana_program_error::ProgramError::InvalidArgument)?;
		if event.discriminator != PROPOSAL_STATUS_EVENT_DISCRIMINATOR {
			return Err(solana_program_error::ProgramError::InvalidArgument);
		}
		if event.migration_version != PROPOSAL_STATUS_EVENT_MIGRATION_VERSION {
			return Err(solana_program_error::ProgramError::InvalidArgument);
		}
		Ok(event)
	}
...
/// Why `ProposalStatusEvent::try_from_bytes` rejected event bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ProposalStatusEventVersionError {
...
impl core::fmt::Display for ProposalStatusEventVersionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid ProposalStatusEvent event data"),
			Self::Stale { stored } => {
				write!(
					f,
...
impl std::error::Error for ProposalStatusEventVersionError {}
...
impl ProposalStatusEvent {
	...
	pub fn try_from_bytes(
		data: &[u8],
	) -> Result<&ProposalStatusEventZc, ProposalStatusEventVersionError> {
		let event = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| ProposalStatusEventVersionError::InvalidData)?;
		if event.discriminator != PROPOSAL_STATUS_EVENT_DISCRIMINATOR {
			return Err(ProposalStatusEventVersionError::InvalidData);
		}
		if event.migration_version > PROPOSAL_STATUS_EVENT_MIGRATION_VERSION {
			return Err(ProposalStatusEventVersionError::Future {
...
/// Why `ProposalStatusEvent::project_from_bytes` could not produce current bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ProposalStatusEventProjectionError {
...
impl core::fmt::Display for ProposalStatusEventProjectionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid ProposalStatusEvent event data"),
...
impl std::error::Error for ProposalStatusEventProjectionError {}

/// Current-shape event bytes paired with the version that actually wrote them.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct ProjectedProposalStatusEvent {
	bytes: Vec<u8>,
	source_version: u8,
	was_migrated: bool,
}

impl ProjectedProposalStatusEvent {
	...
	pub fn data(&self) -> Result<&ProposalStatusEventZc, ProposalStatusEventVersionError> {
		ProposalStatusEvent::try_from_bytes(&self.bytes)
	}
}
...
const PROPOSAL_STATUS_EVENT_PROJECTION_STEPS: &[(
	u32,
	u32,
	bool,
	usize,
...
impl ProposalStatusEvent {
	...
	pub fn project_from_bytes(
		data: &[u8],
	) -> Result<ProjectedProposalStatusEvent, ProposalStatusEventProjectionError> {
		if data.len() < 2 {
			return Err(ProposalStatusEventProjectionError::InvalidData);
		}
		if data[..1] != [1] {
			return Err(ProposalStatusEventProjectionError::InvalidData);
		}
		let mut version = u8::from_le_bytes(
			data[1..2]
				.try_into()
				.map_err(|_| ProposalStatusEventProjectionError::InvalidData)?,
		);
		let expected = PROPOSAL_STATUS_EVENT_MIGRATION_VERSION;
		if version > expected {
			return Err(ProposalStatusEventProjectionError::Unknown {
		...
		while version != expected {
			let Some((from, to, automatic, source_size, destination_size, moves)) =
				PROPOSAL_STATUS_EVENT_PROJECTION_STEPS
					.iter()
			...
			if !*automatic {
				return Err(ProposalStatusEventProjectionError::Manual {
					from: *from,
					to: *to,
				});
			}
			if payload.len() != *source_size {
				return Err(ProposalStatusEventProjectionError::InvalidLength {
					stored: u32::from(version),
				});
			}
			...
			version = u8::try_from(*to)
				.map_err(|_| ProposalStatusEventProjectionError::Unknown { stored: *to })?;
		}
		let mut bytes = Vec::with_capacity(2 + payload.len());
		bytes.extend_from_slice(&PROPOSAL_STATUS_EVENT_DISCRIMINATOR.to_le_bytes()[..1]);
		bytes.extend_from_slice(&0u8.to_le_bytes()[..1]);
		bytes.extend_from_slice(&payload);
		Ok(ProjectedProposalStatusEvent {
			bytes,
			source_version,
...
#[cfg(test)]
mod proposal_status_event_projection_tests {
	use super::*;
	...
	fn future_versions_fail_closed() {
		let future: u8 = 1;
		let error = ProposalStatusEvent::project_from_bytes(&record(future, &[]))
			.err()
			.expect("a future version must fail");
		assert_eq!(
			error,
			ProposalStatusEventProjectionError::Unknown {
	...
	fn wrong_lengths_and_discriminators_fail_closed() {
		...
		assert_eq!(
			ProposalStatusEvent::project_from_bytes(&foreign).err(),
			Some(ProposalStatusEventProjectionError::InvalidData),
		);
	}
}
