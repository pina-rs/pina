...
pub const COUNTER_STATE_MIGRATION_VERSION: u8 = 0u8;
...
impl CounterState {
	...
	pub fn initialize(
		...
		<Self as pina::PinaPodFixed>::initialize(data, |account| {
			...
			account.migration_version = COUNTER_STATE_MIGRATION_VERSION;
			Ok(())
		})
	...
	pub fn from_bytes(data: &[u8]) -> Result<&CounterStateZc, solana_program_error::ProgramError> {
		...
		if account.migration_version != COUNTER_STATE_MIGRATION_VERSION {
			return Err(solana_program_error::ProgramError::InvalidAccountData);
		}
		Ok(account)
	}
...
/// [`COUNTER_STATE_MIGRATION_VERSION`]. Current or foreign bytes return false; decoding explains the difference.
///
/// Version 0 is the initial version, so no bytes can ever be stale.
pub fn counter_state_needs_migration(_data: &[u8]) -> bool {
	false
}

/// Why `CounterState::try_from_bytes` rejected account bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CounterStateVersionError {
...
impl core::fmt::Display for CounterStateVersionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid CounterState account data"),
			Self::Stale { stored } => {
				write!(
					f,
...
impl CounterState {
	/// Decodes current-version bytes and tells stale envelopes (migrate the account) apart from future ones (upgrade this client). The failure message mirrors the generated JavaScript decoder. For the strict current-only convenience returning `ProgramError`, see [`CounterState::from_bytes`].
	pub fn try_from_bytes(data: &[u8]) -> Result<&CounterStateZc, CounterStateVersionError> {
		let account = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| CounterStateVersionError::InvalidData)?;
		if account.discriminator != COUNTER_STATE_DISCRIMINATOR {
			return Err(CounterStateVersionError::InvalidData);
		}
		if account.migration_version > COUNTER_STATE_MIGRATION_VERSION {
			return Err(CounterStateVersionError::Future {
...
#[cfg(test)]
mod counter_state_version_error_tests {
	use super::*;

	fn envelope(version: u8) -> Vec<u8> {
		let mut data = vec![0_u8; core::mem::size_of::<CounterStateZc>()];
	...
	fn stale_and_future_versions_are_distinguishable() {
		let error = CounterState::try_from_bytes(&envelope(1 as u8))
			.err()
			.expect("a future envelope must fail");
		assert_eq!(error, CounterStateVersionError::Future { stored: 1 });
		assert_eq!(
			CounterStateVersionError::Future { stored: 1 }.to_string(),
			"migration version mismatch: expected 0, received 1 (the data was written by a newer \
			 program; upgrade this client)"
		);
		assert!(
			CounterState::try_from_bytes(&envelope(0 as u8)).is_ok(),
