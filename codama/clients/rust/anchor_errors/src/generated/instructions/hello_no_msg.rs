...
impl HelloNoMsgInstructionData {
	pub fn new(
		...
		let mut bytes = vec![0u8; <HelloNoMsgInstructionWire as pina::ZeroPodFixed>::SIZE];
		{
			let data =
				<HelloNoMsgInstructionWire as pina::ZeroPodFixed>::from_bytes_mut(&mut bytes)
					.map_err(|_| solana_program_error::ProgramError::InvalidInstructionData)?;
			data.discriminator = HELLO_NO_MSG_DISCRIMINATOR;
			configure(data);
		}
		<HelloNoMsgInstructionWire as pina::ZeroPodFixed>::validate(&bytes)
			.map_err(|_| solana_program_error::ProgramError::InvalidInstructionData)?;
		Ok(Self { bytes })
	}
}
