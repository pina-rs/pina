...
pub const ORACLE_STATE_MIGRATION_VERSION: u8 = 0u8;
...
impl OracleState {
	...
	pub fn initialize(
		...
		<Self as pina::PinaPodFixed>::initialize(data, |account| {
			...
			account.migration_version = ORACLE_STATE_MIGRATION_VERSION;
			Ok(())
		})
	...
	pub fn from_bytes(data: &[u8]) -> Result<&OracleStateZc, solana_program_error::ProgramError> {
		...
		if account.migration_version != ORACLE_STATE_MIGRATION_VERSION {
			return Err(solana_program_error::ProgramError::InvalidAccountData);
		}
		Ok(account)
	}
...
/// [`ORACLE_STATE_MIGRATION_VERSION`]. Current or foreign bytes return false; decoding explains the difference.
///
/// Version 0 is the initial version, so no bytes can ever be stale.
pub fn oracle_state_needs_migration(_data: &[u8]) -> bool {
	false
}

/// Why `OracleState::try_from_bytes` rejected account bytes.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum OracleStateVersionError {
...
impl core::fmt::Display for OracleStateVersionError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		match self {
			Self::InvalidData => write!(f, "invalid OracleState account data"),
			Self::Stale { stored } => {
				write!(
					f,
...
impl OracleState {
	/// Decodes current-version bytes and tells stale envelopes (migrate the account) apart from future ones (upgrade this client). The failure message mirrors the generated JavaScript decoder. For the strict current-only convenience returning `ProgramError`, see [`OracleState::from_bytes`].
	pub fn try_from_bytes(data: &[u8]) -> Result<&OracleStateZc, OracleStateVersionError> {
		let account = <Self as pina::PinaPodFixed>::read_exact(data)
			.map_err(|_| OracleStateVersionError::InvalidData)?;
		if account.discriminator != ORACLE_STATE_DISCRIMINATOR {
			return Err(OracleStateVersionError::InvalidData);
		}
		if account.migration_version > ORACLE_STATE_MIGRATION_VERSION {
			return Err(OracleStateVersionError::Future {
...
#[cfg(test)]
mod oracle_state_version_error_tests {
	use super::*;

	fn envelope(version: u8) -> Vec<u8> {
		let mut data = vec![0_u8; core::mem::size_of::<OracleStateZc>()];
	...
	fn stale_and_future_versions_are_distinguishable() {
		let error = OracleState::try_from_bytes(&envelope(1 as u8))
			.err()
			.expect("a future envelope must fail");
		assert_eq!(error, OracleStateVersionError::Future { stored: 1 });
		assert_eq!(
			OracleStateVersionError::Future { stored: 1 }.to_string(),
			"migration version mismatch: expected 0, received 1 (the data was written by a newer \
			 program; upgrade this client)"
		);
		assert!(
			OracleState::try_from_bytes(&envelope(0 as u8)).is_ok(),
