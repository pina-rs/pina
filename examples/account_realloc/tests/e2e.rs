//! SBF regressions for the authenticated `account_realloc` lifecycle.
//!
//! Build the program before running these ignored tests:
//!
//! ```sh
//! cargo build-sbf --manifest-path examples/account_realloc/Cargo.toml \
//!     --sbf-out-dir target/deploy --features bpf-entrypoint
//! SBF_OUT_DIR=target/deploy \
//!     cargo test -p account_realloc --test e2e -- --include-ignored
//! ```

use account_realloc::ID;
use account_realloc::InitializeIx;
use account_realloc::Realloc2Ix;
use account_realloc::ReallocError;
use account_realloc::ReallocIx;
use account_realloc::Sample;
use account_realloc::SamplePatch;
...
fn create_mollusk() -> Mollusk {
	let so_name = "account_realloc.so";
	...
	assert!(
		...
		"account_realloc SBF binary not found; build it before running ignored e2e tests"
	);

	Mollusk::new(&program_id(), "account_realloc")
}
...
fn realloc_ix_data(len: usize) -> Vec<u8> {
	...
	.unwrap_or_else(|error| panic!("account_realloc instruction encoding failed: {error:?}"));
	data
}
...
#[test]
#[ignore = "requires the account_realloc SBF binary"]
