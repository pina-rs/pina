//! SBF regressions for the authenticated `account_realloc_program` lifecycle.
//!
//! Build the program before running these ignored tests:
//!
//! ```sh
//! cargo build-sbf --manifest-path examples/account_realloc_program/Cargo.toml \
...
fn create_mollusk() -> Mollusk {
	let so_name = "account_realloc_program.so";
	...
	assert!(
		...
		"account_realloc_program SBF binary not found; build it before running ignored e2e tests"
	);

	Mollusk::new(&program_id(), "account_realloc_program")
}
...
fn realloc_ix_data(len: usize) -> Vec<u8> {
	...
	.unwrap_or_else(|error| {
		panic!("account_realloc_program instruction encoding failed: {error:?}")
	});
	data
}
...
#[test]
#[ignore = "requires the account_realloc_program SBF binary"]
