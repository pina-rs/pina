...
The earlier assertion style — `assert!(!error.message().is_empty())` — proved only that *something* failed, which passes even when the program fails for the wrong reason.
