...
The end-to-end assertion lives outside the measured suites for now. The Surfpool runtime that CI exercises rejects the v1 config mask this client train encodes (`invalid transaction config mask`), while the local Surfpool accepts it, so a v1 transaction that executes locally cannot yet be asserted portably. The construction, signing, and wire-encoding path is covered hermetically in `pina_test`, including that a v1 transaction carries instruction data past the 1,232-byte legacy limit while staying inside the 4,096-byte v1 limit.
