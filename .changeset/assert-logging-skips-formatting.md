...
The saving is small — a few hundred bytes in a program that actually reaches the path — because the logger never linked `core::fmt` in the first place. The message is still logged in every configuration; only how it is assembled changed.
