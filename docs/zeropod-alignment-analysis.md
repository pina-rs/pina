...
Fixed-layout schemas require fully initialized bounded arrays like these (this mirrors the real `ProfileState` in `examples/profile_program`). Compact schemas may instead end in one or more trailing bounded `String`/`PodString` and `Vec`/`PodVec` tails under `#[account(compact)]`.
