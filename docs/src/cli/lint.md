...
The lint code is ordinary Rust compiled into a `pina_lint_driver` binary that ships prebuilt next to the `pina` CLI itself. Release targets whose runner can host the pinned nightly archive both binaries side by side, and the npm platform packages for those targets carry them into their `bin/` directory; the remaining targets ship CLI-only archives. The CLI never builds or installs anything — on a supported platform it runs the driver beside its own executable.
...
The command exits successfully only when the selected driver resolves and loads, compilation finishes, and all enabled security lints succeed. A run without `PINA_LINT_DRIVER_PATH` selects the bundled prebuilt driver and additionally fails on a missing driver or a toolchain mismatch; a diagnostic at an error level, a compilation failure, or an invalid `PINA_LINT_DRIVER_PATH` produces a non-zero exit in every mode. Child Cargo output stays attached to the terminal; Pina prints a short completion summary only after success.
