...
Instruction arguments support little-endian signed and unsigned integers (`i8`–`i128`, `u8`–`u128`), booleans, public keys, fixed `u8` arrays, and PinaPod bounded strings and vectors. Floating-point and compact `shortU16` numbers are rejected rather than rendered with a different wire format.
