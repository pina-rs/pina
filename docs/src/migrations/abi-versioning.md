...
```text
crates/pina_abi/fixtures/v0.20/manifest.json
crates/pina_abi/fixtures/v0.20/publications.json
crates/pina_abi/fixtures/v0.20/manifest.schema.json
crates/pina_abi/fixtures/v0.20/publications.schema.json
crates/pina_abi/fixtures/current/…
```

- `v<version>/` directories are frozen when that release ships and are never regenerated. They are the only artifacts that record what an older release actually wrote; a fixture regenerated from current code would encode today's shape under yesterday's version and prove nothing.
