...
- with the `floats` feature: `f32` and `f64`, stored as the bit pattern of their backing little-endian integer, and fixed-point `FixedI*<Frac>`/`FixedU*<Frac>` types from the pinned `fixed` crate. See [Float and Fixed-Point Fields](./floats.md).
