...
Direct Pinapod derives and manual `PinaAccount` / `ZeroPodFixed` implementations remain available for advanced integration, but they are outside Pina's audited macro-generated contract. Their authors must uphold Pinapod's complete bit-validity, alignment, initialization, size, validation-order, and aliasing invariants.
