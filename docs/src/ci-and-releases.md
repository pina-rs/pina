...
The main CI workflow also runs `release-publish` on every pull request. When a PR contains releaseable changesets, the job creates the same release commit as the production release workflow, keeps that commit local to the runner, and dry-runs package publication from its embedded release record. Prepared release PRs are checked directly. Pull requests without a publishable release keep the job visible but skip the dry-run explicitly.
...
Lint libraries are then built natively because Dylint compiler plugins are tied to the host, pinned nightly toolchain, and dynamic loader. Rust's musl hosts cannot produce the required `cdylib`, so the musl CLI packages select the active Rust compiler's GNU bundle when used on glibc Linux; a truly musl-hosted compiler is reported as unsupported. FreeBSD lint libraries run in a pinned VM action. The workflow waits for all CLI and lint assets, attests and verifies every `pina-*` asset, and only then publishes packages and makes the GitHub release public.

`crates/pina_cli/lints.json` is the single release contract shared by the CLI and bundle builders. `toolTargets` lists executable hosts, while `lintTargets` lists hosts capable of loading rustc-private dynamic libraries. Its `dylintVersion` field is the pointer to the reusable tool release; changing it causes the next publish run to create that release if it does not already exist. Tests require the catalog to match every crate under `lints/`, the repository Rust toolchain, and the workspace's Dylint dependency version. The installed CLI selects releases by semantic version rather than embedding a commit revision, so automated releases remain a single merge-and-publish flow.
