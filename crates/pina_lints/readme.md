...
- **Its failure stops the handler.** A `Result`/`Option` guard is propagated with `?`, extracted with `unwrap()`/`expect()`, returned, or tested by a `match`, `if let`, or `let ... else` whose branches for the failing value return `Err`/`None` (or evaluate to one when the whole expression is itself returned or propagated) or panic. Failure-preserving adapters (`map_err`, `inspect_err`, `map`, `and_then`, `and`, `ok`, `ok_or`, `or(Err(..))`) keep the guard intact; `or(Ok(..))`, `unwrap_or`, and a discarded result do not. A `bool` guard, or `is_err()`/`is_ok()` of a fallible one, must gate an `if` whose failing branch returns `Err`/`None` or panics — `assert!`, `assert_eq!`, and `pina::assert(ok, error, message)?` qualify. A branch that returns `Ok`, `break`s, or `continue`s is not a failure, so `if guard().is_ok() { return Ok(()) }` does not gate the drain that follows. A guard bound to a local counts where that local is enforced, unless the local is reassigned or mutably borrowed first.
...
- **It names the check, or delegates to one.** The name has to contain `pause`, `cap`, `circuit`, `halt`, `guard`, `limit`, or `throttle`, because behavior alone cannot tell a cap check from `assert_signer()?`, which every handler propagates. A differently named local function that returns `Result`/`Option` counts when its own body enforces such a guard in its outermost scope before any non-error `return`, followed up to three wrappers deep, so `enforce_withdrawal_policy(&config)?` above is accepted. A wrapper returning `bool` is never followed, because its polarity is unknown.
- **It can fail.** A local callee whose every returned value is a constant `Ok(..)`/`Some(..)` (or a `bool` literal) and that cannot otherwise fail or panic is not a guard, whatever its name. Trait method calls are judged by the implementation that runs, never by a default body that the implementation overrides; in a generic handler where the implementation cannot be resolved, only the method name is used.

Known limits: a `bool` guard's polarity is unknown, so a failing branch on either side of its `if` counts. Callees from other crates are judged by their name and call-site behavior only. A local guard-named callee counts if anything in its body can fail, even for reasons unrelated to its claimed check. Wrappers deeper than three levels are not followed. A pause check with no guard-named call, such as `if state.paused { return Err(..) }` or a differently named helper taking only the flag, is not recognized.
