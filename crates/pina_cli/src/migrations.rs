...
fn manual_transition_source(
	...
	let (requirement, migrate) = match identity.kind {
		ContractKind::Account => {
			(
	...
	format!(
		"// Manual adjacent ABI migration generated by `pina migrations make`.\n// Source \
		 ...
		 ({destination_size} bytes)\n// TODO(pina-manual-migration): {requirement}.\npub(crate) \
		 const SOURCE_SIZE: usize = {source_size};\npub(crate) const DESTINATION_SIZE: usize = \
		 {destination_size};\npub(crate) const WORKING_SIZE: usize = {working_size};\n\n{migrate}",
		source.version,
	)
}
