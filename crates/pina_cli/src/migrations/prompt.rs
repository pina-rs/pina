...
pub struct MigrationAnswers {
	...
	/// Added fields whose value the developer converts by hand instead of
	/// accepting a generated byte move or zero fill.
...
impl MigrationAnswers {
	...
	/// Collect answers from repeatable CLI arguments, including manual fields.
	pub fn from_flags_with_manual(
		renames: &[String],
		removed: &[String],
		manual: &[String],
		no_interactive: bool,
	) -> Result<Self, String> {
		Self::parse_answers(renames, removed, manual, no_interactive)
	}
	...
	pub fn from_layers(
		...
		Self::from_layers_with_manual(persisted, renames, removed, &[], no_interactive)
	}

	/// [`Self::from_layers`] including manual-conversion answers.
	pub fn from_layers_with_manual(
		persisted: &crate::project::MigrationsAnswersConfig,
		renames: &[String],
		removed: &[String],
		manual: &[String],
		...
		let mut answers = Self::parse_answers(
			&persisted.rename,
			&persisted.assume_removed,
			&persisted.manual,
			no_interactive,
		)?;
		let flag_answers = Self::parse_answers(renames, removed, manual, no_interactive)?;
		for field in &flag_answers.removed {
			...
			let conflict = if answers.manual.contains(field) {
				...
				answers
					.renames
					.get(field)
					.map(|persisted_to| format!("rename `{field}:{persisted_to}`"))
			};
			if let Some(conflict) = conflict {
				return Err(format!(
					"`--assume-removed {field}` contradicts the persisted {conflict} in \
					 pina.toml; update one of them"
				));
			}
		}
		// A flag rename replaces a persisted manual answer for the same target:
		// the developer just said the bytes move verbatim, not that they are
		// converted, and silently honouring the stale flag would ignore them.
		for to in flag_answers.renames.values() {
			answers.manual.remove(to.as_str());
		}
		// A flag manual answer replaces a persisted rename for the same target
		// for the same reason, in the other direction.
		for field in &flag_answers.manual {
			answers
				.renames
				.retain(|_, target| target.as_str() != field.as_str());
		}
		// A persisted removal and a flag manual answer are the same
		// contradiction as the flag pair: one discards the stored bytes the
		// other needs. Checked in both directions so neither source of answers
		// can silently win.
		for field in &flag_answers.manual {
		...
		answers.manual.extend(flag_answers.manual);
		Ok(answers)
	}
