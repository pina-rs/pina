...
pub const LINT_REFERENCE: &[LintExplanation] = &[
	...
	LintExplanation {
		...
		contract: "When an integer snapshot of a token balance is taken before a value-moving \
		           ...
		           either sit in the same statement as a post-CPI reload that runs on every path \
		           to it (as in `after.checked_sub(before)`) or compare the snapshot against a \
		           constant. A custody-named transfer destination (`vault`, `custody`, `reserve`, \
		           or `pool`) must also be read both before and after every transfer, with no \
		...
		rationale: "Token-2022 transfer fees can make the amount received differ from the amount \
		            ...
		            received.",
		blessing: "Reload the destination with `amount()` after the CPI and compute the delta in \
		           the statement that uses the snapshot. Comparing the snapshot against a \
		           constant (`if prior == 0`) records a fact about the earlier state and is \
		           accepted as is. A static `invoke()` called on a builder bound to the legacy \
		           `pinocchio_token` program is exempt because that program cannot charge a fee; \
		           any other exception needs a narrowly scoped `#[allow]` with the reason the \
