...
fn verify_typescript_scaffold_kit_major(
	...
	let floor = (MINIMUM_GENERATED_KIT_MAJOR, 0);
	if range_reaches_floor(range, floor) == Some(false) {
...
/// Decide whether an npm dependency range can resolve a version at or above
/// the generated floor.
///
/// Ranges arrive as comparator sets (`^8.3.0`, `>=8 <9`, `8.x`), unions of
/// them (`^8.3.0 || ^7.0.0`), or tags carrying no version (`workspace:*`).
/// What matters is whether the range can *reach* the floor: `>=7 <9` permits
/// 8.x and is accepted, `^7.0.0` stops at 7.x and is rejected, and a union is
/// accepted when any alternative reaches the floor because npm installs the
/// highest satisfying version.
fn range_reaches_floor(range: &str, floor: (u16, u16)) -> Option<bool> {
	let mut saw_version = false;

	for alternative in range.split("||") {
		match alternative_ceiling(alternative) {
			Some(Ceiling::Unbounded) => return Some(true),
			Some(Ceiling::Below(maximum)) => {
				saw_version = true;
				if maximum > floor {
					return Some(true);
				}
			}
			Some(Ceiling::AtMost(maximum)) => {
				saw_version = true;
				if maximum >= floor {
					return Some(true);
				}
	...
	if saw_version { Some(false) } else { None }
}

/// The highest version a comparator set can resolve.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Ceiling {
	/// No upper bound at all (a bare lower bound such as `>=8`).
	Unbounded,
	/// Everything strictly below this version (`^7.0.0`, `<8`).
	Below((u16, u16)),
	/// Everything up to and including this version (`<=8`, `8.1.2`).
	AtMost((u16, u16)),
}

/// Combine the comparators of one union alternative into its ceiling.
fn alternative_ceiling(alternative: &str) -> Option<Ceiling> {
	let mut ceiling: Option<Ceiling> = None;

	for comparator in alternative.split_whitespace() {
		let Some(found) = comparator_ceiling(comparator) else {
			continue;
		};
		ceiling = Some(match ceiling {
			None => found,
			Some(current) => minimum_ceiling(current, found),
		});
	}
...
/// Keep whichever of two ceilings is the tighter upper bound.
fn minimum_ceiling(left: Ceiling, right: Ceiling) -> Ceiling {
	match (left, right) {
		(Ceiling::Unbounded, other) | (other, Ceiling::Unbounded) => other,
		(Ceiling::Below(left), Ceiling::Below(right)) => Ceiling::Below(left.min(right)),
		(Ceiling::AtMost(left), Ceiling::AtMost(right)) => Ceiling::AtMost(left.min(right)),
		(Ceiling::Below(below), Ceiling::AtMost(at_most))
		| (Ceiling::AtMost(at_most), Ceiling::Below(below)) => {
			if below <= at_most {
				Ceiling::Below(below)
			} else {
				Ceiling::AtMost(at_most)
			}
		}
	}
}

/// The ceiling a single comparator imposes, or `None` when it carries no
/// readable version (`*`, `workspace:*`, `latest`).
fn comparator_ceiling(comparator: &str) -> Option<Ceiling> {
	let comparator = comparator.trim();
	if comparator.is_empty() || comparator.starts_with('*') || comparator.eq_ignore_ascii_case("x")
	{
		return None;
	}

	// A lower bound leaves the range open above.
	if comparator.starts_with('>') {
		version_pair(comparator)?;
		return Some(Ceiling::Unbounded);
	}

	if comparator.starts_with('<') {
		let version = version_pair(comparator)?;
		return Some(if comparator.starts_with("<=") {
			Ceiling::AtMost(version)
		} else {
			Ceiling::Below(version)
		});
	}

	if comparator.starts_with('^') {
		let (major, minor) = version_pair(comparator)?;
		// Caret allows changes that do not modify the leftmost non-zero digit.
		return Some(Ceiling::Below(if major > 0 {
			(major + 1, 0)
		} else {
			(0, minor + 1)
		}));
	...
	if comparator.starts_with('~') {
		let (major, minor) = version_pair(comparator)?;
		return Some(Ceiling::Below((major, minor + 1)));
	}

	// Wildcards and bare or exact versions.
	let raw = comparator.trim_start_matches(['=', 'v', ' ']);
	let segments: Vec<&str> = raw.split('.').collect();
	let wildcard = segments
		.iter()
		.position(|segment| segment.eq_ignore_ascii_case("x"));

	match (segments.len(), wildcard) {
		(_, Some(0)) => Some(Ceiling::Unbounded),
		(2.., Some(1)) => {
			let (major, _) = version_pair(raw)?;
			Some(Ceiling::Below((major + 1, 0)))
		}
		_ => Some(Ceiling::AtMost(version_pair(raw)?)),
	}
}

/// Read a leading `major.minor` pair, tolerating a missing or wildcard minor
/// (`>=8`, `8.x`) and any operator or sigil prefix.
///
/// A segment carrying digits that do not fit the field is unreadable rather
/// than zero, so callers leave it unpoliced.
fn version_pair(value: &str) -> Option<(u16, u16)> {
...
fn verify_dart_scaffold_kit_range(
	...
	for (name, range) in dart_kit_ranges(&pubspec) {
		if range_reaches_floor(range, MINIMUM_GENERATED_KIT_DART) == Some(false) {
			return Err(CodamaError::StaleKitScaffold {
...
/// Collect every `solana_kit_*` dependency range from a pubspec, covering
/// both the inline (`solana_kit_addresses: ^0.8.0`) and the nested-mapping
/// (`solana_kit_addresses:` / `version: ^0.8.0`) declaration forms.
fn dart_kit_ranges(pubspec: &str) -> Vec<(&str, &str)> {
	let mut ranges = Vec::new();
	// The Kit package whose nested block the current line belongs to.
	let mut nested: Option<&str> = None;
	...
	for line in pubspec.lines() {
		...
		if name.starts_with("solana_kit_") {
			if value.is_empty() {
				// A nested block; its `version:` line carries the range.
				nested = Some(name);
			} else {
				ranges.push((name, value));
				nested = None;
			}
			continue;
		}

		// Inside a Kit block, `version:` supplies the range.
		if indented
			&& name == "version"
			&& let Some(package) = nested.take()
		{
			ranges.push((package, value));
		}
	}
...
mod tests {
	...
	/// Write a pubspec that declares a Kit package in the nested form, which
	/// carries its range on a separate `version:` line.
	fn write_nested_pubspec(dir: &Path, range: &str) {
		write_manifest(
			dir,
			"pubspec.yaml",
			format!(
	...
	#[test]
	fn ranges_are_judged_by_what_they_can_resolve() {
		let kit_floor = (MINIMUM_GENERATED_KIT_MAJOR, 0);

		// Ceilings that stop below the floor.
		for rejected in ["^7.0.0", " ~6.10.0 ", ">=7 <8", "7.x"] {
			assert_eq!(
				range_reaches_floor(rejected, kit_floor),
				Some(false),
				"{rejected} must be rejected"
			);
		}

		// Ceilings that can reach it, including unions and bounded ranges.
		for accepted in [
			"^8.3.0",
			">=8",
			"8.x",
			...
			assert_eq!(
				range_reaches_floor(accepted, kit_floor),
				Some(true),
				"{accepted} must be accepted"
			);
		}

		// Ranges carrying no version are never policed.
		for unknown in ["workspace:*", "*", "latest", ""] {
			assert_eq!(
				range_reaches_floor(unknown, kit_floor),
				None,
				"{unknown} must be unjudged"
			);
		}
	}

	#[test]
	fn every_ceiling_shape_is_combined_by_its_tightest_bound() {
		let floor = (8, 0);

		// Upper bounds combine: the tighter of two ceilings wins. Each of
		// these windows spans 8.0 except the empty one.
		assert_eq!(range_reaches_floor(">=8 <=8", floor), Some(true));
		assert_eq!(range_reaches_floor("<9 <8.5", floor), Some(true));
		assert_eq!(range_reaches_floor(">=8 <8.5", floor), Some(true));
		assert_eq!(range_reaches_floor(">=8 <=7.9", floor), Some(false));

		// Exclusive upper bounds sit just below the version they name, so
		// `<=8` reaches the 8.0 floor while `<8` does not.
		assert_eq!(range_reaches_floor("<=8", floor), Some(true));
		assert_eq!(range_reaches_floor("<8", floor), Some(false));

		// Wildcards: `*` and `x` prove nothing, `x` as the major spans every
		// version, a wildcard minor spans its major, and a bare exact version
		// caps at itself.
		assert_eq!(range_reaches_floor("x", floor), None);
		assert_eq!(range_reaches_floor("x.1", floor), Some(true));
		assert_eq!(range_reaches_floor("8.x", floor), Some(true));
		assert_eq!(range_reaches_floor("7.x", floor), Some(false));
		assert_eq!(range_reaches_floor("8.1.2", floor), Some(true));
		assert_eq!(range_reaches_floor("~8.1", floor), Some(true));

		// Two inclusive caps: the lower one wins, in both orderings.
		assert_eq!(range_reaches_floor("<=9 <=8.5", floor), Some(true));
		assert_eq!(range_reaches_floor("<=8.5 <=7.5", floor), Some(false));

		// An inclusive cap tighter than an exclusive one wins, and the
		// reverse combination keeps the exclusive bound.
		assert_eq!(range_reaches_floor("<9 <=8.5", floor), Some(true));
		assert_eq!(range_reaches_floor("<7 <=8", floor), Some(false));
	}

	#[test]
	fn dart_ceilings_below_the_kit_floor_are_rejected() {
		let floor = MINIMUM_GENERATED_KIT_DART;

		// A capped range that excludes every release at the floor, even
		// though its own bound sits just below it.
		assert_eq!(range_reaches_floor(">0.9.0 <0.10.0", floor), Some(false));
		assert_eq!(range_reaches_floor("^0.8.0", floor), Some(false));

		// Ranges that reach the floor, and rangeless constraints.
		assert_eq!(range_reaches_floor(">=0.10.0 <1.0.0", floor), Some(true));
		assert_eq!(range_reaches_floor(">0.9.0", floor), Some(true));
		assert_eq!(range_reaches_floor("any", floor), None);
	}
	#[test]
	fn nested_pubspec_kit_dependencies_are_read_from_their_version_field() {
		let temp =
			tempfile::TempDir::new().unwrap_or_else(|error| panic!("temp dir failed: {error}"));
		let client = temp.path().join("dart");
		write_nested_pubspec(&client, "^0.8.0");
		...
		assert!(
			...
			"a nested Kit 0.8 dependency must block regeneration"
		);

		// The same shape on the current range passes.
		write_nested_pubspec(&client, "\">=0.10.0 <1.0.0\"");
		assert!(
			verify_dart_scaffold_kit_range(&client.join("pubspec.yaml"), settings).is_ok(),
			"a nested current dependency must pass"
		);
	}

	#[test]
	fn generated_dart_sdk_floor_matches_the_kit_requirement() {
		// Solana Kit Dart 0.10 requires Dart ^3.13.0, so neither generated
		// manifest may advertise an older SDK. Both templates embed the same
		// SDK floor, which the render script carries as a literal.
		let floors = CLIENT_RENDER_SCRIPT
			.match_indices("sdk: \">=3.13.0 <4.0.0\"")
			.count();

		assert_eq!(
			floors, 2,
			"the Dart client and Dart CLI scaffolds must both floor their SDK at 3.13"
		);
	}
