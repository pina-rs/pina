...
//! # Instruction data
//!
//! [`InstructionBuilder::data`] returns a deterministic, fully initialized byte
//! representation of an instruction struct (including its discriminator).

use crate::Address;
use crate::HasDiscriminator;
use crate::PinaToBytes;
...
/// // `HasDiscriminator` and `PinaToBytes`.
...
impl<T: HasDiscriminator + PinaToBytes> InstructionBuilder<T> {
	/// Returns the initialized fixed-layout representation of `instruction`.
	///
	...
	#[inline]
	pub fn data(instruction: &T) -> T::Bytes {
...
mod tests {
	...
	impl PinaSerialize for TestInstruction {
		fn write_bytes(&self, output: &mut [u8]) {
			assert_eq!(output.len(), core::mem::size_of::<Self>());
			self.discriminator.write_bytes(&mut output[..1]);
			self.amount.write_bytes(&mut output[1..]);
		}
	}

	impl PinaToBytes for TestInstruction {
		type Bytes = [u8; core::mem::size_of::<Self>()];

		fn zeroed_bytes() -> Self::Bytes {
			[0u8; core::mem::size_of::<Self>()]
		}
	}
