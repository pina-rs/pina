...
///
/// # Returns
/// * `Ok([u8; N])` - A fixed-size byte array containing the string data,
///   zero-padded if needed
/// * `Err(ProgramError)` - Returns ERROR_STRING_TOO_LONG if input string is
///   longer than N bytes
...
///
/// # Returns
/// * `Ok(String)` - The converted string, truncated at the first null byte if
///   present
/// * `Err(ProgramError)` - Returns ERROR_INVALID_UTF8 if the bytes are not
///   valid UTF-8
