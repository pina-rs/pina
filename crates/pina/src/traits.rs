...
use crate::RefMut;
#[cfg(feature = "compact")]
use crate::ZeroPodCompact;
use crate::ZeroPodFixed;
use crate::impls::validate_writable;

/// A native account schema with a zeropod-generated zero-copy representation.
///
/// `Self` is the ergonomic Rust schema declared with [`crate::account`]. It is
/// never reinterpreted as account memory. Zeropod derives a separate
/// [`ZeroPodFixed::Zc`] companion (for example, `CounterStateZc`) and owns the
...
/// that its [`ZeroPodFixed`] implementation upholds zeropod's bit-validity,
...
/// ```
pub trait PinaAccount: HasDiscriminator + ZeroPodFixed {
	/// Validate the discriminator and content of `data`.
	fn validate_account_data(data: &[u8]) -> Result<(), ProgramError> {
		if data.len() != Self::SIZE || !Self::matches_discriminator(data) {
			return Err(ProgramError::InvalidAccountData);
		}

		<Self as ZeroPodFixed>::validate(data).map_err(|_| ProgramError::InvalidAccountData)
	}

	/// Validate `data`, then borrow zeropod's immutable zero-copy companion.
	fn try_from_bytes(data: &[u8]) -> Result<&Self::Zc, ProgramError> {
		<Self as PinaAccount>::validate_account_data(data)?;
		<Self as ZeroPodFixed>::from_bytes(data).map_err(|_| ProgramError::InvalidAccountData)
	}

	/// Validate `data`, then borrow zeropod's mutable zero-copy companion.
	fn try_from_bytes_mut(data: &mut [u8]) -> Result<&mut Self::Zc, ProgramError> {
		<Self as PinaAccount>::validate_account_data(data)?;
		<Self as ZeroPodFixed>::from_bytes_mut(data).map_err(|_| ProgramError::InvalidAccountData)
	}
}
...
/// The associated view types are generated by `#[account(compact)]` and may
/// not outlive the bytes from which they were created. Generated account types
/// expose `MIN_SIZE`, `MAX_SIZE`, one `*_CAPACITY` constant per tail, and a
/// count-aware `projected_bytes` constructor. Their views expose the committed
/// logical `encoded_size`; mutable views additionally inherit Pinapod's
/// `projected_size` for staged tail replacements.
#[cfg(feature = "compact")]
pub trait PinaCompactAccount: HasDiscriminator + ZeroPodCompact {
	/// Immutable compact view generated by zeropod.
	type Ref<'data>
	where
		Self: 'data;

	/// Mutable compact view generated by zeropod.
	type Mut<'data>
	where
		Self: 'data;

	/// Maximum byte length accepted by the declared compact schema.
	const MAX_SIZE: usize;

	/// Greatest common byte alignment shared by every compact tail element.
	...
	fn validate_size(size: usize) -> Result<(), ProgramError> {
		if Self::TAIL_ALIGNMENT == 0
			...
			|| size > Self::MAX_SIZE
			|| !(size - Self::HEADER_SIZE).is_multiple_of(Self::TAIL_ALIGNMENT)
		{
			return Err(ProgramError::InvalidAccountData);
		}
	...
	fn validate_account_data(data: &[u8]) -> Result<(), ProgramError> {
		...
		<Self as ZeroPodCompact>::validate(data).map_err(|_| ProgramError::InvalidAccountData)
	}

	/// Validate and borrow an immutable compact view.
	fn try_from_bytes(data: &[u8]) -> Result<Self::Ref<'_>, ProgramError>;

	/// Validate and borrow a mutable compact view.
	fn try_from_bytes_mut(data: &mut [u8]) -> Result<Self::Mut<'_>, ProgramError>;
...
pub trait AsCompactAccount {
	...
	/// Load a mutable compact account view for the duration of `use_account`.
	///
	/// Tail setters update the view; call the generated view's `commit` method
	/// before the closure returns to persist its length prefix and elements.
...
mod tests {
	...
	fn account_deserialize_oversized_data() {
		...
		data[0] = 7;
		let result = TestType::try_from_bytes(&data);
		// zeropod::ZeroPodFixed::from_bytes rejects slices that aren't exactly the right
		// size.
		assert!(result.is_err());
	}
