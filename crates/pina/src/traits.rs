...
///
/// Compact accounts keep inline fields and a collection length in a fixed
/// header, followed by only the active collection elements. The associated
/// view types are generated by `#[account(compact)]` and may not outlive the
/// bytes from which they were created.
pub trait PinaCompactAccount: HasDiscriminator + ZeroPodCompact {
	...
	/// Byte width of one element in the trailing compact collection.
	const TAIL_ELEMENT_SIZE: usize;

	/// Validate that `size` can hold this compact account.
	fn validate_size(size: usize) -> Result<(), ProgramError> {
		if Self::TAIL_ELEMENT_SIZE == 0
			|| size < Self::HEADER_SIZE
			|| size > Self::MAX_SIZE
			|| !(size - Self::HEADER_SIZE).is_multiple_of(Self::TAIL_ELEMENT_SIZE)
		{
			return Err(ProgramError::InvalidAccountData);
		}
