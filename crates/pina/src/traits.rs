...
///
/// Validates the discriminator and content, then reinterprets the byte slice
/// as `&Self` (or `&mut Self`) without copying. Implemented by the `#[account]`
/// macro for fixed-layout account types.
///
/// # Examples
///
/// ```ignore
...
/// let account = MyAccount::from_bytes(&data)
///     .unwrap_or_else(|e| panic!("deserialization failed: {e:?}"));
/// assert_eq!(u64::from(account.value), 42);
/// ```
pub trait PinaAccount: HasDiscriminator {
	/// Validate the discriminator and content of `data`.
	fn validate(data: &[u8]) -> Result<(), ProgramError>;

	/// Validate the discriminator and content, then reinterpret `data` as `&Self`.
	fn try_from_bytes(data: &[u8]) -> Result<&Self, ProgramError>;

	/// Validate the discriminator and content, then reinterpret `data` as `&mut Self`.
...
mod tests {
	...
	impl ZcValidate for TestType {
		fn validate_ref(value: &Self) -> Result<(), ZeroPodError> {
			<[u8; 1] as ZcValidate>::validate_ref(&value.discriminator)?;
			PodU64::validate_ref(&value.field0)?;
			PodU64::validate_ref(&value.field1)?;
			Ok(())
		}
	}
	...
	unsafe impl ZcElem for TestType {}

	impl ZeroPodSchema for TestType {
		const LAYOUT: crate::LayoutKind = crate::LayoutKind::Fixed;
	}

	impl ZeroPodFixed for TestType {
		type Zc = TestType;

		const SIZE: usize = core::mem::size_of::<TestType>();
	...
	impl ZcField for TestType {
		type Pod = TestType;

		const POD_SIZE: usize = core::mem::size_of::<TestType>();
	}
	...
	impl PinaAccount for TestType {
		...
		fn try_from_bytes(data: &[u8]) -> Result<&Self, ProgramError> {
			<Self as PinaAccount>::validate(data)?;
		...
		fn try_from_bytes_mut(data: &mut [u8]) -> Result<&mut Self, ProgramError> {
			...
			<Self as ZeroPodFixed>::from_bytes_mut(data)
	...
	fn account_deserialize() {
		...
		assert_eq!(42u64, u64::from(foo.field0));
		assert_eq!(43u64, u64::from(foo.field1));
	}
	...
	fn account_deserialize_mut_allows_modification() {
		...
		let foo = TestType::try_from_bytes_mut(&mut data).unwrap();
		foo.field0 = PodU64::from(999);
		foo.field1 = PodU64::from(888);
