...
/// The separate zeroing step that `require_zeroed_before_close` accepts and
/// recommends before a non-zeroing close, written exactly as its help text
/// spells it.
fn close_after_zero_fill(account: &mut AccountView, recipient: &mut AccountView) -> ProgramResult {
	account.try_borrow_mut()?.fill(0);
	account.close_with_recipient(&TEST_PROGRAM_ID, recipient)
}

#[test]
fn zero_fill_then_close_with_recipient_clears_source_bytes() {
	let account_key: Address = address!("2Eg4H7V2Cd9uBSXreMKe1KjEo9e4NMpM4GZpsLtkj6pp");
	let recipient_key: Address = address!("9Z6iYoJ1E9nQ7h6nC3ieUzYDRKsAqPPYUajF2iGaGLjm");
	let state_data = build_test_state_bytes(9, 99);
	...
	let result = close_after_zero_fill(closed_account, recipient);
	assert!(result.is_ok(), "close should succeed: {result:?}");

	let source_bytes = unsafe {
		// SAFETY: the serialized test input buffer remains allocated for the
		// duration of this test, and closing only rewrites the account metadata.
		core::slice::from_raw_parts(source_ptr, source_len)
	};
