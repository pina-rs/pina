...
- Some suites cannot run under instrumentation: `trybuild` compares compiler output byte for byte, and `crates/pina_cli/tests/generated_surfpool.rs` and `pina_root`'s `tests/ui.rs` are excluded with `#![cfg(not(coverage))]` for that reason. Excluding a suite from coverage does not remove it from `test:all`; both must stay in step.
